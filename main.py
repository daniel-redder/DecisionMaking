...
if play_tournament:
    ...
    brett_ai = next(ai for ai in ais if ai.get_name() == "Bretts AI")
    percy_ai = next(ai for ai in ais if ai.get_name() == "Percy")
    brill_ai = next(ai for ai in ais if ai.get_name() == "Brill")

    # We set up and play a single game...
    ais = [(brett_ai,percy_ai),(percy_ai,brett_ai),(brett_ai,brill_ai),(brill_ai,brett_ai),(brill_ai,percy_ai),(brill_ai,percy_ai)]
    for player1, player2 in ais:
        for x in range(100):
            Logger.add_handler(FileLogHandler(f"data/{player1.get_name()}_{player2.get_name()}_{x}.log", Logger.INFO))
            game = Game()
