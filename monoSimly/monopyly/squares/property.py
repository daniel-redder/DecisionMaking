...
class Property(Square):
    ...
    async def landed_on(self, game, player):
        ...
        if self.owner is player:
            await Logger.log("{0} already owns this property".format(player.name))
            return

        # If the property is mortgaged, then there is nothing to do...
        if self.is_mortgaged:
            await Logger.log("Property is mortgaged")
            return

        if self.owner is None:
            # The property is not owned, so we offer it for sale...
            await game.offer_property_for_sale(player, self)
        else:
            # The property is owned by another player (and is not
            # mortgaged), so rent needs to be paid...
            await self._pay_rent(game, player)

    async def _pay_rent(self, game, player):
        '''
        ...
        rent = await self.calculate_rent(game, player)

        # We take the rent from the player, and give it to the
        # player who owns the square...
        await Logger.log("{0} must pay rent of £{1} to {2}".format(player.name, rent, self.owner.name))
        await game.transfer_cash(player, self.owner, rent, Game.Action.PAY_AS_MUCH_AS_POSSIBLE)
