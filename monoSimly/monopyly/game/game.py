...
class Game(object):
    ...
    def __init__(self):
        ...
        self.eminent_domain_round = 28

    async def add_player(self, ai_info):
        '''
        Adds a player AI.
    ...
    async def play_game(self):
        ...
        await Logger.log("Start of game. Players:")
        await Logger.indent()
        for player in self.state.players:
            await player.call_ai( player.ai.start_of_game)
    ...
    async def play_one_round(self):
        ...
        await self._check_eminent_domain_rule()
        ...
        for player in list_players:
            ...
            await self.play_one_turn(player)

            # We notify the GUI (via the Tournament object)...
            if self.tournament is not None:
                await self.tournament.turn_played(self)

            # If there is only one player left, the game is over...
            if len(self.state.players) == 1:
                break

    async def play_one_turn(self, current_player):
        '''
        Plays one turn for one player.
        '''
        # Some logging...
        await Logger.log("")
        await Logger.log("Start of turn for {0}".format(current_player.name))
        await Logger.indent()
        await Logger.log("Cash=£{0}, Net Worth=£{1}".format( current_player.state.cash, await current_player.net_worth))
        ...
        for player in self.state.players:
            await player.call_ai(player.ai.start_of_turn, self.state, current_player)

        # The player can make deals...
        await self._make_deals(current_player)

        # If the player is in jail, they have a chance to buy their way out...
        await self._get_out_of_jail(current_player)

        # The player can unmortgage properties...
        await self._unmortgage_properties(current_player)

        # The player can build houses...
        await self._build_houses(current_player)
        ...
        while roll_again == Game.Action.ROLL_AGAIN:
            roll_again, number_of_doubles_rolled = await self.roll_and_move(current_player, number_of_doubles_rolled)
    ...
    async def roll_and_move(self, current_player, number_of_doubles_rolled):
        '''
        ...
        if current_player.state.is_in_jail :
            if not doubles_rolled and current_player.state.number_of_turns_in_jail < 3:
                ...
                await self.take_money_from_player(current_player, 50)
        ...
        if doubles_rolled:
            ...
            if number_of_doubles_rolled == 3:
                await self.send_player_to_jail(current_player)
        ...
        await self.player_has_changed_square(current_player)
    ...
    async def player_has_changed_square(self, current_player):
        '''
        ...
        await Logger.log("Moved to {0}".format(square.name))

        # We notify all players that the player has landed
        # on this square...
        for player in self.state.players:
            await player.call_ai(player.ai.player_landed_on_square, self.state, square, current_player)

        # We perform the square's landed-on action...
        await square.landed_on(self, current_player)

    async def take_money_from_player(self, player, amount):
        '''
        ...
        await player.call_ai(player.ai.money_will_be_taken, player, amount)
        ...
        if not self._in_make_deals:
            await self._make_deals(player)

        # We allow the player to sell houses...
        await self._sell_houses(player)

        # We allow the player to mortgage properties...
        await self._mortgage_properties(player)

        # We take the money, and tell the player that it was taken...
        cash_before_money_taken = player.state.cash
        player.state.cash -= amount
        await player.call_ai(player.ai.money_taken, player, amount)

        # And log it...
        await Logger.log("{0} pays £{1}".format(player.name, amount))
    ...
    async def give_money_to_player(self, player, amount):
        '''
        Gives some money to the player.
        '''
        # We give the money to the player and tell them about it...
        player.state.cash += amount
        await player.call_ai(player.ai.money_given, player, amount)

        # And log it...
        await Logger.log("{0} gets £{1}".format(player.name, amount))

    async def transfer_cash(self, from_player, to_player, amount, action):
        '''
        ...
        if action == Game.Action.PAY_AS_MUCH_AS_POSSIBLE:
            ...
            await self.give_money_to_player(to_player, amount_taken)
        elif action == Game.Action.ROLLBACK_ON_INSUFFICIENT_CASH:
            # We roll back the transfer...
            await self.give_money_to_player(from_player, amount)

        return Game.Action.TRANSFER_FAILED

    async def send_player_to_jail(self, player):
        '''
        ...
        await self.player_has_changed_square(player)

    async def give_property_to_player(self, player, square_name):
        '''
        Gives a named property to a player.

        Returns the property square.
        '''
        await Logger.log("{0} gets {1}".format(player.name, square_name))
    ...
    async def transfer_property(self, from_player, to_player, square_name):
        '''
        ...
        await Logger.log("{0} transfered from {1} to {2}".format(
    ...
    async def offer_property_for_sale(self, current_player, square):
        '''
        ...
        action = await self._offer_property_to_current_player(current_player, square)
        if action == Game.Action.PROPERTY_BOUGHT:
            return

        # The current player did not buy the property, so we put it
        # up for auction...
        await self._offer_property_for_auction(square)

    async def _offer_property_for_auction(self, square):
        '''
        ...
        highest (and so on).
        '''
        await Logger.log("{0} offered for auction".format(square.name), Logger.INFO)
        await Logger.indent()
        ...
        for player_who_won_auction in self.state.players:
            ...
            bid = await player_who_won_auction.call_ai(
        ...
        await Logger.log("Bids: {0}".format(readable_bids))
        ...
        for i in range(number_of_bids):
            ...
            await self.take_money_from_player(player_who_won_auction, selling_price)
            if player_who_won_auction.state.cash < 0:
                ...
                await self.give_money_to_player(player_who_won_auction, selling_price)
            else:
                # The player successfully bought the property...
                await self.give_property_to_player(player_who_won_auction, square.name)
                status = PlayerAIBase.Action.AUCTION_SUCCEEDED
                break

        # We notify all the players...
        for player in self.state.players:
            await player.call_ai(player.ai.auction_result, status, square, player_who_won_auction, selling_price)

        await Logger.dedent()

    async def _offer_property_to_current_player(self, player, square):
        '''
        ...
        action = await player.call_ai(player.ai.landed_on_unowned_property, self.state, player, square)
        ...
        await self.take_money_from_player(player, square.price)

        if player.state.cash < 0:
            # The player did not have enough money, so we revert the transaction...
            await self.give_money_to_player(player, square.price)
            return Game.Action.PROPERTY_NOT_BOUGHT
        else:
            # The purchase was successful...
            await self.give_property_to_player(player, square.name)
    ...
    async def _build_houses(self, current_player):
        '''
        ...
        build_instructions = await current_player.call_ai(current_player.ai.build_houses, self.state, current_player)
        if not build_instructions:
            return

        await Logger.log("{0} wants to build houses: {1}".format(current_player.name, build_instructions))
        await Logger.indent()
        ...
        await self._build_houses_and_take_money(current_player, build_instructions)

        # Did the player have enough money?
        if current_player.state.cash < 0:
            await Logger.log("{0} does not have enough cash".format(current_player.name))
            await  self._roll_back_house_building(current_player, build_instructions)
            await Logger.dedent()
            return
        ...
        for (street, number_of_houses) in build_instructions:
            ...
            if number_of_houses < 0:
                await Logger.log("Negative number of houses specified")
                await self._roll_back_house_building(current_player, build_instructions)
                await Logger.dedent()
                return
            ...
            if street.number_of_houses > 5:
                await Logger.log("Too many houses requested")
                await self._roll_back_house_building(current_player, build_instructions)
                await Logger.dedent()
                return
            ...
            if street.property_set not in current_player.state.owned_unmortgaged_sets:
                await Logger.log("Set now fully owned, or partly mortgaged")
                await self._roll_back_house_building(current_player, build_instructions)
                await Logger.dedent()
                return
            ...
            if not self._set_has_balanced_houses(street.property_set):
                await Logger.log("Building was unbalanced")
                await self._roll_back_house_building(current_player, build_instructions)
                await Logger.dedent()
                return

        await Logger.log("Houses built successfully")
        await Logger.dedent()

    async def _set_has_balanced_houses(self, property_set):
        '''
    ...
    async def _build_houses_and_take_money(self, current_player, build_instructions):
        '''
        ...
        # And take the money...
        await self.take_money_from_player(current_player, total_cost)

    async def _roll_back_house_building(self, current_player, build_instructions):
        '''
        ...
        await self.give_money_to_player(current_player, total_cost)

    async def _mortgage_properties(self, current_player):
        '''
        We give the current player the option to mortgage properties.
        '''
        # We ask the player if they want to mortgage anything...
        properties_to_mortgage =await current_player.call_ai(
        ...
        await self.give_money_to_player(current_player, total_mortgage_value)
        await self._update_sets()

    async def _sell_houses(self, current_player):
        '''
        We offer the player the chance to sell houses.
        '''
        # We ask the player which properties they want to sell...
        sale_instructions =await current_player.call_ai(
        ...
        for (street, number_of_houses) in sale_instructions:
            ...
            if number_of_houses < 0:
                await self._replace_houses(sale_instructions)
                return
        ...
        await self.give_money_to_player(current_player, sale_value)

    async def _remove_houses(self, instructions):
        '''
    ...
    async def _replace_houses(self, instructions):
        '''
    ...
    async def _get_out_of_jail(self, current_player):
        '''
        ...
        action =await current_player.call_ai(
    ...
    async def _unmortgage_properties(self, current_player):
        ...
        if current_player.state.cash < 0:
            ...
            # transaction...
            await self.give_money_to_player(current_player, unmortgage_cost)
        else:
    ...
    async def _make_deals(self, current_player):
        '''
        The player can make up to three deals.
        '''
        self._in_make_deals = True
        for i in range(3):
            await self._make_deal(current_player)
        self._in_make_deals = False

    async def _make_deal(self, current_player):
        '''
        Lets the player propose a deal.
        '''
        # We see if the player wants to propose a deal...
        proposal = await current_player.call_ai(
        ...
        await Logger.log("{0} proposed deal to {1}: {2}".format(current_player.name, proposed_to_player.name, proposal))
        await Logger.indent()
        ...
        if (response is None) or (response.action == DealResponse.Action.REJECT):
            ...
            await current_player.call_ai(current_player.ai.deal_result, PlayerAIBase.DealInfo.DEAL_REJECTED)
            await proposed_to_player.call_ai(proposed_to_player.ai.deal_result, PlayerAIBase.DealInfo.DEAL_REJECTED)
            await Logger.log("{0} rejected the deal".format(proposed_to_player.name))
            await Logger.dedent()
            return
        ...
        if minimum_cash_wanted > 0:
            ...
            if response.maximum_cash_offered < minimum_cash_wanted:
                await current_player.call_ai(current_player.ai.deal_result, PlayerAIBase.DealInfo.ASKED_FOR_TOO_MUCH_MONEY)
                await proposed_to_player.call_ai(proposed_to_player.ai.deal_result, PlayerAIBase.DealInfo.OFFERED_TOO_LITTLE_MONEY)
                await Logger.log("Deal rejected: {0} asked for more than {1} was willing to pay".format(
            ...
            if maximum_cash_offered < response.minimum_cash_wanted:
                await current_player.call_ai(current_player.ai.deal_result, PlayerAIBase.DealInfo.OFFERED_TOO_LITTLE_MONEY)
                await proposed_to_player.call_ai(proposed_to_player.ai.deal_result, PlayerAIBase.DealInfo.ASKED_FOR_TOO_MUCH_MONEY)
                await Logger.log("Deal rejected: {0} offered less than {1} was willing to accept".format(
                    current_player.name, proposed_to_player.name))
                await Logger.dedent()
                return
        ...
        if result == Game.Action.TRANSFER_FAILED:
            await current_player.call_ai(current_player.ai.deal_result, PlayerAIBase.DealInfo.PLAYER_DID_NOT_HAVE_ENOUGH_MONEY)
            await proposed_to_player.call_ai(proposed_to_player.ai.deal_result, PlayerAIBase.DealInfo.PLAYER_DID_NOT_HAVE_ENOUGH_MONEY)
            await Logger.log("Deal accepted, but player did not have enough money")
            await Logger.dedent()
            return

        # The cash transfer worked, so we transfer the properties...
        await Logger.log("Deal successful")
        for property in proposal.properties_offered:
            await self.transfer_property(current_player, proposed_to_player, property.name)

        for property in proposal.properties_wanted:
            await self.transfer_property(proposed_to_player, current_player, property.name)

        # We tell the players that the deal succeeded...
        await current_player.call_ai(current_player.ai.deal_result, PlayerAIBase.DealInfo.SUCCEEDED)
        await proposed_to_player.call_ai(proposed_to_player.ai.deal_result, PlayerAIBase.DealInfo.SUCCEEDED)
        ...
        for player in self.state.players:
            await player.call_ai(player.ai.deal_completed, deal_result)
    ...
    async def _check_for_bankrupt_players(self):
        '''
        ...
        for player in list_players:
            if player.state.cash < 0:
                await self._player_went_bankrupt(player)

    async def _player_went_bankrupt(self, current_player):
        '''
        Called when a player has gone bankrupt.
        '''
        await Logger.log("{0} went bankrupt".format(current_player.name))

        # We notify all players that this player went bankrupt...
        for player in self.state.players:
            await player.call_ai(player.ai.player_went_bankrupt, current_player)

        # We remove the player from the game...
        await self._remove_player(current_player)

    async def _player_ran_out_of_time(self, current_player):
        '''
        Called when a player has run out of time.
        '''
        await Logger.log("{0} ran out of time".format(current_player.name))

        # We notify all players that this player went bankrupt...
        for player in self.state.players:
            await player.call_ai(player.ai.player_ran_out_of_time, current_player)

        # We remove the player from the game...
        await self._remove_player(current_player)

    async def _remove_player(self, player):
        '''
    ...
    async def _check_eminent_domain_rule(self):
        '''
        ...
        for property in properties:
            ...
            await self.give_money_to_player(owner, price)
            property.owner = None
            property.is_mortgaged = False
        ...
        for player in self.state.players:
            await player.call_ai(player.ai.eminent_domain, self.state, player)

        # Now we auction all properties...
        for property in properties:
            await self._offer_property_for_auction(property)
